...
type ColumnType = ColumnDef<RentalOrderInterface, unknown>;

interface RentalOrderListPageProps {
...
export function RentalOrderListPage(props: RentalOrderListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RentalOrderInterface>>(
    () => `/rental-orders?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RentalOrderInterface) => {
    if (hasAccess('rental_order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/rental-orders/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/rental-orders/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="rental_order" />
        </Flex>
