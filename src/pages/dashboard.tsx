...
import { getRentalOrders } from 'apiSdk/rental-orders';
...
function DashboardPage() {
  ...
  const rentalOrderStat = useSWR<PaginatedInterface<RentalOrderInterface>>(
    () => `/rental-orders`,
    () => getRentalOrders({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Rental Orders',
      result: rentalOrderStat?.data?.totalCount,
      isLoading: rentalOrderStat.isLoading,
